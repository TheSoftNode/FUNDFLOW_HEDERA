    ...
    Pausable
{
    ...
    function getCampaignsByCategory(
        ...
        return _categoryCampaigns[category];
    }
    ...
    function getFeaturedCampaigns()
        ...
        returns (uint256[] memory)
    {
        // Count currently featured campaigns
        uint256 featuredCount = 0;
        uint256 totalCampaigns = fundFlowCore.getTotalCampaigns();

        for (uint256 i = 1; i <= totalCampaigns; i++) {
            if (_featuredCampaigns[i] && block.timestamp <= _featuredUntil[i]) {
                featuredCount++;
            }
        }

        // Create array of featured campaigns
