...
async function continueDeployment() {
    try {
        console.log("🚀 Continuing FundFlow Smart Contract Deployment");
        ...
        deployments.fundFlowCore = {
            contractId: "0.0.6558178",
            contractAddress: AccountId.fromString("0.0.6558178").toSolidityAddress(),
            success: true
        };

        deployments.campaignManager = {
            contractId: "0.0.6558181",
            contractAddress: AccountId.fromString("0.0.6558181").toSolidityAddress(),
            success: true
        };

        console.log("✅ Using already deployed contracts:");
        console.log(`   FundFlowCore: ${deployments.fundFlowCore.contractId}`);
        console.log(`   CampaignManager: ${deployments.campaignManager.contractId}`);
        ...
        const investmentManagerParams = [
            deployments.fundFlowCore.contractAddress, // fundFlowCore address
        ...
        const registerCampaignManager = await deployer.callContractFunction(
            deployments.fundFlowCore.contractId,
            "setCampaignManager",
            [deployments.campaignManager.contractAddress],
            200000
        );
        ...
        const registerInvestmentManager = await deployer.callContractFunction(
            ...
            [deployments.investmentManager.contractAddress],
            200000
        );
        ...
        const registerMilestoneManager = await deployer.callContractFunction(
            deployments.fundFlowCore.contractId,
            "setMilestoneManager",
            [deployments.milestoneManager.contractAddress],
            200000
        );
        ...
        const registerAnalyticsManager = await deployer.callContractFunction(
            deployments.fundFlowCore.contractId,
            "setAnalyticsManager",
            [deployments.analyticsManager.contractAddress],
            200000
        );
        ...
        const registerGovernanceManager = await deployer.callContractFunction(
            ...
            [deployments.governanceManager.contractAddress],
            200000
        );
